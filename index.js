...
//1. GET a random joke
// Math.foolr round the number and math.random give a random number between 0 and 1
...
app.get("/filter", (req, res) =>{
  ...
  const filteredActivities = jokes.filter((jokes) => joke.jokeType === type);
...
app.patch("jokes/:id", (req, res) =>{
  ...
  const replacementJoke = {
    // || stand for or it first check if a user pass a text then grap it otherwise grap from existing jokeText
...
app.delete("/jokes/:id", (req, res) =>{
  ...
  const searchIndex = jokes.findIndex((joke) => joke.id === id);
  if (searchIndex > -1) {
    // splice will delete the item and next argument will look at how many item should delete.
